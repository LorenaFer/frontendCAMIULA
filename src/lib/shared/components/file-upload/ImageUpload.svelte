...
<div class="w-full {className}">
    ...
    <div class="relative group">
      ...
      >
        <button
          type="button"
          onclick={handleClick}
          class="p-2 bg-white rounded-lg text-slate-700 hover:bg-slate-100 transition-colors"
        >
  ...
  {:else}
    ...
    >
      <div class="rounded-full p-2 bg-slate-100">
      ...
      <p class="text-sm text-slate-600">Upload image</p>
      {#if maxSize}
        <p class="text-xs text-slate-400">Max {formatFileSize(maxSize)}</p>
      {/if}
    </div>
  {/if}
