...
<div class="w-full" bind:this={containerEl}>
	...
	<div
		...
		role="group"
		class="
			min-h-9 rounded-lg border bg-surface-elevated px-2 py-1.5
			flex flex-wrap items-center gap-1.5
			transition-all duration-150
			{getStateClasses(error, isOpen)}
			{disabled ? 'bg-canvas-subtle cursor-not-allowed' : 'cursor-text'}
			{className}
		"
	>
		{#if showTags}
			{#each selectedOptions as option (option.value)}
				<span
					class="inline-flex items-center gap-1 px-2 py-0.5 bg-canvas-subtle text-ink text-sm rounded"
				>
					{option.label}
					{#if !disabled}
						<button
							...
							class="p-0.5 hover:bg-canvas-subtle rounded transition-colors"
						>
							<svg
								class="w-3 h-3"
								fill="none"
		...
		{#if searchable}
			<input
				...
				{disabled}
				autocomplete="off"
				class="flex-1 min-w-20 bg-transparent text-sm text-ink placeholder:text-ink-subtle outline-none"
				{...restProps}
			/>
		{:else}
			<span
		...
		<svg
			class="w-4 h-4 text-ink-subtle flex-shrink-0 transition-transform {isOpen
				? 'rotate-180'
				: ''}"
			fill="none"
			viewBox="0 0 24 24"
		...
		<Portal>
			...
			>
				{#if filteredOptions.length === 0}
					<li class="px-3 py-2 text-sm text-ink-muted">No results found</li>
				{:else}
						...
						>
							<div
								class="
									...
									{isSelected ? 'bg-slate-900 border-slate-900' : 'border-border-strong'}
								"
							>
								{#if isSelected}
									<svg
