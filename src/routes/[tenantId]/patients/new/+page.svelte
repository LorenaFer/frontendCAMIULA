...
<main class="p-6 lg:p-8 bg-canvas">
  <section class="max-w-6xl mx-auto space-y-6">
    ...
    <form method="POST" class="space-y-6">
      {#if form?.error}
        <Card variant="ghost" class="border-honey-300/70 bg-honey-50/70 shadow-sm" padding="sm">
          <CardDescription class="text-honey-800 dark:text-honey-300">{form.error}</CardDescription>
        </Card>
      {/if}

      {#if form?.success}
        <Card variant="ghost" class="border-sage-300/70 bg-sage-50/70 shadow-sm" padding="sm">
      ...
      <Card padding="none">
        <CardHeader class="px-5 py-4 border-b border-border-subtle/70">
          <CardTitle class="flex items-center gap-2 text-sm font-semibold">
            <span class="inline-flex w-6 h-6 items-center justify-center rounded-md bg-viking-50 text-viking-600" aria-hidden="true">
        ...
        <CardBody class="px-5 py-5">
          <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <Select
              ...
              bind:value={values.relacion_univ}
      ...
      <Card padding="none">
        ...
        <CardBody class="px-5 py-5">
          <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            <Select name="sexo" label="Sexo" bind:value={values.sexo} options={sexoOptions} required error={form?.fieldErrors?.sexo} />
            <Input name="edad" label="Edad" bind:value={values.edad} placeholder="00" />
            <DateInput label="Fecha de nacimiento" value={values.fecha_nacimiento} oninput={(e) => values.fecha_nacimiento = e.target.value} />
      ...
      <Card padding="none">
        <CardHeader class="px-5 py-4 border-b border-border-subtle/70">
          <CardTitle class="flex items-center gap-2 text-sm font-semibold">
            <span class="inline-flex w-6 h-6 items-center justify-center rounded-md bg-sage-50 text-sage-600" aria-hidden="true">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <CardBody class="px-5 py-5">
          <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            <Select name="estado_civil" label="Estado civil" bind:value={values.estado_civil} options={estadoCivilOptions} />
            <Select name="grupo_sanguineo" label="Grupo sanguineo" bind:value={values.grupo_sanguineo} options={sangreOptions} />
      ...
      <Card padding="none">
        <CardHeader class="px-5 py-4 border-b border-border-subtle/70">
          <CardTitle class="flex items-center gap-2 text-sm font-semibold">
            <span class="inline-flex w-6 h-6 items-center justify-center rounded-md bg-honey-50 text-honey-600" aria-hidden="true">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <CardBody class="px-5 py-5 space-y-4">
          <Textarea name="direccion_habitacion" label="Direccion de habitacion completa" bind:value={values.direccion_habitacion} rows={2} placeholder="Direccion de domicilio" />
          <Textarea name="direccion_trabajo" label="Direccion de trabajo" bind:value={values.direccion_trabajo} rows={2} placeholder="Direccion laboral" />
        </CardBody>
      </Card>
      ...
      <Card padding="none">
        <CardHeader class="px-5 py-4 border-b border-border-subtle/70">
          <CardTitle class="flex items-center gap-2 text-sm font-semibold">
            <span class="inline-flex w-6 h-6 items-center justify-center rounded-md bg-viking-50 text-viking-700" aria-hidden="true">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <Card padding="none">
        <CardHeader class="px-5 py-4 border-b border-border-subtle/70">
          <CardTitle class="flex items-center gap-2 text-sm font-semibold">
            <span class="inline-flex w-6 h-6 items-center justify-center rounded-md bg-iris-50 text-iris-600" aria-hidden="true">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <CardBody class="px-5 py-5 space-y-4">
          ...
          <Textarea
            ...
            bind:value={values.descripcion_alergias}
            rows={2}
          ...
          <Textarea
            ...
            bind:value={values.otras_alertas}
            rows={2}
      ...
      <Card padding="none">
        <CardFooter class="px-5 py-4 justify-end">
          <div class="flex items-center gap-2">
            <Button type="button" variant="secondary" onclick={() => goto(`/${$page.params.tenantId}/patients`)}>
              Cancelar
            </Button>
...
{#snippet headerAction()}
  <Button
    ...
    onclick={() => goto(`/${$page.params.tenantId}/patients`)}
  >
    Volver al listado
  </Button>
{/snippet}
